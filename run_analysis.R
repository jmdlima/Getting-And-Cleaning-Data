...
# Checks if the directory and data exists
# Downloads and unzips the data if not present in the folder 
...
vars<-names(variablesdf)     # stores the names of the measurements for later use
...
#applies the ddply function 

consolidated %>% melt(id.vars=c("subjectID","Activity_Desc"),measure.vars = vars) %>%
 ddply(.(subjectID, Activity_Desc, variable), summarize, mean = mean(value)) %>%
 dcast(subjectID + Activity_Desc ~ variable,value.var="mean") %>%
   write.table("Tidy.txt",sep=",",row.names=F)
